...
async def upload_mapping(file: UploadFile = File(...), midiapp: MidiApp = Depends(get_midiapp)):
    if midiapp.running:
        return Response(
            ...
            status_code=400,
            headers={
                "X-Toast": "Stop the app before loading a new mapping",
                "X-Toast-Type": "error",
            },
        )
    ...
    try:
        ...
        return Response(
            ...
            status_code=500,
            headers={
                "X-Toast": f"Failed to load mapping: {str(e)}",
                "X-Toast-Type": "error",
            },
        )
...
async def start_show(midiapp: MidiApp = Depends(get_midiapp)):
    ...
    if editor_state.dirty:
        return Response(
            ...
            status_code=400,
            headers={
                "X-Toast": "Save changes before starting",
                "X-Toast-Type": "error",
            },
        )
...
async def request_restart():
    ...
    return Response(content="", headers={"X-Toast": "Restarting...", "X-Toast-Type": "info"})


@router.get("/healthz")
async def healthz(version: str = Depends(get_current_version)):
    return {"alive": True, "version": version, "pid": os.getpid()}
